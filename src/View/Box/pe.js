...
export default (function () {
    ...
    return {
                ...
                for(let capability of result) {
                    for(let valueOf of capability.valueOf()) {
                        ...
                        if( perform_name === 'THUNDER_DEFENCE') {
                            ...
                            perform_value = valueOf[0].indexOf('防雷器') > -1 ? [valueOf[0], valueOf[1]] : [valueOf[0], valueOf[3]];
        ...
        methods: {
            ...
            handlerDisabledReset(vol, index) {
                if([5, 6].indexOf(index) > -1 || index === 4 && !vol[6]) {
                    return { 'pointer-events' : "none", "cursor" : "no-drop", "color" : "#909399 !important" };
                }
                return {};
            },
            //设备类型下拉框
            ...
            async queryBoxInfo(deviceId) {
                ...
                if (response.body.success) {
                    ...
                    let powerNameList = queryDevPerformMap.filter(it => it.PERFORM_NAME.indexOf("PER_OUTPUT_POWER_NAME") > -1);
                    let PER_OUTPUT_POWER_NAME = [];
                    if(powerNameList.length) {
                        let it = powerNameList[0];
                        PER_OUTPUT_POWER_NAME =  CapabilityFactory.getInstance().createCapability(it.PERFORM_NAME, { 
                            id : it.ID, 
                            perform_name : it.PERFORM_NAME, 
                            perform_type : it.PERFORM_TYPE, 
                            data_type : it.DATA_TYPE, 
                            perform_value : it.PERFORM_VALUE, 
                            perform_description : it.PERFORM_DESCRIPTION }).valueOf();
                    } else {
                        PER_OUTPUT_POWER_NAME = [];
                    }

                    let fan_mode = queryDevPerformMap.filter(it => it.PERFORM_NAME.indexOf("OUT_POWER_STATUS") > -1); //风扇模式
                    ...
                    let [[, n1] = [], [, n2] = [], [, n3] = [], [, n4] = [], [, n5] = [], [, n6] = [], [, n7] = [], [, n8] = [], [, n9] = []] = PER_OUTPUT_POWER_NAME || [];
                    let devicePower = [
                        [index + 0, v1, a1, s1, a1 !== undefined || v1 !== undefined || s1 !== undefined, n1],
                        [index + 1, v2, a2, s2, a2 !== undefined || v2 !== undefined || s2 !== undefined, n2],
                        [index + 2, v3, a3, s3, a3 !== undefined || v3 !== undefined || s3 !== undefined, n3],
                        [index + 3, v4, a4, s4, a4 !== undefined || v4 !== undefined || s4 !== undefined, n4],
                        [index + 4, v5, a5, s5, a5 !== undefined || v5 !== undefined || s5 !== undefined, n5, fan_mode && fan_mode.PERFORM_VALUE === '0' || false], //风扇手动模式
                        [index + 5, v6, a6, s6, a6 !== undefined || v6 !== undefined || s6 !== undefined, n6],
                        [index + 6, v7, a7, s7, a7 !== undefined || v7 !== undefined || s7 !== undefined, n7],
                        [index + 7, v8, a8, s8, a8 !== undefined || v8 !== undefined || s8 !== undefined, n8]
                    ];
        ...
        mounted(){
            ...
            me.loadDeviceType();
            // let roleId = me.User.role.id;
            // console.info(me.deviceInfoBox.onlineStatus);
            // if(roleId == 'op' || !me.deviceInfoBox.onlineStatus){ //说明是操作员或者设备不在线
            //     me.userRole = true;
            // }
            me.queryBoxInfo(me.id);
